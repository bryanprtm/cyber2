...
interface RfiScannerProps {
  onScanComplete?: (result: any) => void;
}

export default function RfiScanner({ onScanComplete }: RfiScannerProps) {
  ...
  const [scanRemoteHosts, setScanRemoteHosts] = useState<boolean>(true);
  ...
  const handleScan = async () => {
    ...
    addCommandLine(`Starting RFI vulnerability scan for ${normalizedUrl}...`);
    ...
    try {
      const response = await axios.post('/api/security/rfi-scanner', {
        url: normalizedUrl,
      ...
      if (response.data.success) {
        ...
        addSuccessLine('RFI scan completed');
        
        if (result.vulnerable) {
          addErrorLine(`WARNING: ${normalizedUrl} is VULNERABLE to Remote File Inclusion!`);
          ...
          addSuccessLine(`No RFI vulnerabilities detected in ${normalizedUrl}`);
        }
  ...
  const handleReset = () => {
    ...
    setError(null);
    setScanProgress(0);
    addCommandLine('Reset RFI scanner');
  };
  ...
  return (
    <div className="space-y-6">
      <Card className="p-4 border-primary/30 bg-card">
        <h2 className="text-xl font-tech text-primary mb-4">Remote File Inclusion Scanner</h2>
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-xs font-mono text-muted-foreground mt-1">
              Enter the URL of the page that might be vulnerable to RFI
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                <Checkbox 
                  id="scan-remote" 
                  checked={scanRemoteHosts}
                  onCheckedChange={(checked) => setScanRemoteHosts(!!checked)}
                  disabled={isScanning}
                />
                <Label 
                  htmlFor="scan-remote" 
              ...
              <p className="text-xs font-mono text-muted-foreground ml-6">
                Tests for inclusions of remote files and domains
              </p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Textarea
              id="custom-payloads"
              placeholder="http://example.com/shell.txt\nhttp://attacker.com/payload.php\ndata:text/plain;base64,PD9waHAgcGhwaW5mbygpOyA/Pg=="
            ...
            />
            <p className="text-xs font-mono text-muted-foreground mt-1">
              Add custom RFI payloads, one per line (optional)
            </p>
          </div>
          ...
          {isScanning && (
            <div className="space-y-2">
              <div className="flex justify-between text-xs font-mono">
                <span>Scanning for RFI vulnerabilities...</span>
          ...
          <Tabs defaultValue="summary" className="w-full">
            ...
            <TabsContent value="summary" className="space-y-4">
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="text-lg font-tech text-secondary">RFI Scan Results</h3>
              ...
              <div className="p-4 bg-background rounded-md border border-secondary/20">
                ...
                <div className="space-y-2 text-sm font-mono">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span>{scanResults.testedEndpoints}</span>
                  </div>
                ...
                {scanResults.vulnerable && (
                  <div className="mt-4 p-3 bg-red-500/10 rounded-md border border-red-500/30 flex items-start gap-2">
                    ...
                    <div>
                      <p className="text-sm font-tech text-red-500">RFI Vulnerability Detected!</p>
              ...
              {scanResults.successfulPayloads.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-8">
                  ...
                  <p className="text-sm font-mono text-center max-w-md text-muted-foreground">
                    No successful RFI payloads were detected during the scan.
                    This suggests the target is not vulnerable to Remote File Inclusion attacks.
                  </p>
                </div>
              )}
            </TabsContent>
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-mono mt-1">
                          This application has a Remote File Inclusion vulnerability that could allow attackers to:
                        </p>
                        <ul className="list-disc list-inside text-xs font-mono mt-2 space-y-1">
                          <li>Include malicious files from remote servers</li>
                          <li>Execute arbitrary code on the server</li>
                          <li>Access or modify sensitive data</li>
                          <li>Potentially gain full control of the web server</li>
                        </ul>
                      </div>
                    </div>
                  </div>
...
// Validate URL to prevent remote file inclusion
function isValidLocalPath(filepath) {
  // Check if it's a URL (remote file)
  try {
    const parsedUrl = new URL(filepath);
    return false; // It's a URL, reject it
  } catch(e) {
    // Not a URL, continue checking local path
  }
  ...
  const sanitizedFile = path.basename(filepath);
  
  // Construct full path
  const fullPath = path.join(CONTENT_DIR, sanitizedFile);
  
  // Verify the path is within allowed directory
  return fullPath.startsWith(CONTENT_DIR);
}`}
                    </pre>
                  </div>
                </div>
